...
dropseq.integrated <- readRDS("/storage/CTR-Projects/CTR_DropSeq/NewMegatron/NewAlignments_50bp/HARMONY_SEPARATE_R_I/harmony_matrix.su_umap_n.neigh25_repuls2_spread2L_rm0004_9617_N729_all_clustree.Rds")
...
#dropseq.integrated <- RunUMAP(dropseq.integrated, reduction = "harmony", dims = 1:30, min.dist = 0.01, n.neighbors = 25, repulsion.strength = 2, spread = 2L)
#dropseq.integrated <- FindNeighbors(dropseq.integrated, reduction = "harmony", dims = 1:30) 
#dropseq.integrated <- FindClusters(dropseq.integrated, resolution = 1)
#dropseq.integrated <- FindClusters(dropseq.integrated, resolution = 1.5)
#dropseq.integrated <- FindClusters(dropseq.integrated, resolution = 2)
#dropseq.integrated <- FindClusters(dropseq.integrated, resolution = 0.6)
#dropseq.integrated <- FindClusters(dropseq.integrated, resolution = 0.8)
#dropseq.integrated <- FindClusters(dropseq.integrated, resolution = 0.2)
#dropseq.integrated <- FindClusters(dropseq.integrated, resolution = 0)
#dropseq.integrated <- FindClusters(dropseq.integrated, resolution = 3)
...
pdf(paste("harmony_orig_matrix.su_clustree2.pdf", sep=""), width=12,height=8)
par(bg=NA)
tree_to_plot
dev.off()
...
dataset_name <- "Harmony.orig_25_rep2_spr2_n729rm" #"dropseq.integrated"
head(dropseq.integrated@meta.data,2)
...
umap.Cluster    <- ggplot(matrix.umap, aes(x=UMAP_1, y=UMAP_2, colour=Cluster)) +
  ...
  theme_classic() +
  annotate('text', label = paste0('0'), x =2.25516244155043, y = 0.362465856906551, color = 'black', size=8) + annotate('text', label = paste0('1'), x =-3.87465361374742, y = -0.504363747003895, color = 'black', size=8) + annotate('text', label = paste0('10'), x =-3.97257117051012, y = 1.05139899098553, color = 'black', size=8) + annotate('text', label = paste0('11'), x =-0.460205114067855, y = 2.73278409087338, color = 'black', size=8) + annotate('text', label = paste0('12'), x =-3.45385864991075, y = -2.24941867745243, color = 'black', size=8) + annotate('text', label = paste0('13'), x =4.3908615944969, y = 2.32498854243435, color = 'black', size=8) + annotate('text', label = paste0('14'), x =1.6068211434471, y = 0.288710547920841, color = 'black', size=8) + annotate('text', label = paste0('15'), x =2.56059988719099, y = 3.1854205712048, color = 'black', size=8) + annotate('text', label = paste0('16'), x =-2.71652749794847, y = 0.801447986003536, color = 'black', size=8) + annotate('text', label = paste0('17'), x =6.29257889968031, y = 1.15466689908184, color = 'black', size=8) + annotate('text', label = paste0('18'), x =-5.1434936882866, y = -0.334199340704304, color = 'black', size=8) + annotate('text', label = paste0('19'), x =-5.95905903595811, y = 3.11363464438595, color = 'black', size=8) + annotate('text', label = paste0('2'), x =-1.35234156865007, y = -1.77110690033756, color = 'black', size=8) + annotate('text', label = paste0('20'), x =4.37039729338759, y = 1.49211281382718, color = 'black', size=8) + annotate('text', label = paste0('21'), x =2.32895644885176, y = 2.44651847922482, color = 'black', size=8) + annotate('text', label = paste0('22'), x =5.50465079527968, y = 0.572559355136532, color = 'black', size=8) + annotate('text', label = paste0('23'), x =-2.66898409146196, y = -0.439906002643925, color = 'black', size=8) + annotate('text', label = paste0('24'), x =0.531355359970269, y = -1.78770935690723, color = 'black', size=8) + annotate('text', label = paste0('25'), x =2.22672959548109, y = 0.589745758411068, color = 'black', size=8) + annotate('text', label = paste0('3'), x =3.26403244715804, y = 1.55446541154065, color = 'black', size=8) + annotate('text', label = paste0('4'), x =2.96854635459059, y = -2.11889296925388, color = 'black', size=8) + annotate('text', label = paste0('5'), x =-1.60892013329393, y = -0.129839691955071, color = 'black', size=8) + annotate('text', label = paste0('6'), x =-4.75586203354723, y = -1.06645953810535, color = 'black', size=8) + annotate('text', label = paste0('7'), x =4.84933587294692, y = -0.0730827093752442, color = 'black', size=8) + annotate('text', label = paste0('8'), x =1.01739475634569, y = 1.91055577838101, color = 'black', size=8) + annotate('text', label = paste0('9'), x =0.132410549937424, y = -0.0244564951094208, color = 'black', size=8)
...
markerDir2 <- "/storage/CTR-Projects/CTR_DropSeq/NewMegatron/NewAlignments_50bp/HARMONY.orig_together_MARKERS/RESULTS_res1.0_l2fc0.25"
setwd(markerDir2)
...
save_res_tables <- function(x,y){
  ...
  if (is.data.frame(y[[x]]$result) == FALSE){ print(0)
    ...
    if(y[[x]] == "same cluster"){  print(0)
      ...
      tmp_df <- subset(tmp_df, tmp_df$p_val_adj < 0.05)
      tmp_df <- subset(tmp_df, abs(tmp_df$avg_logFC) > l2fc_cutoff)
      write.csv(tmp_df, paste0("CTR_dscj1_NewAlign51_HARMONY.orig_" , reso, "_l2fc", l2fc_cutoff,"_Markers_tbl__clusters_", name_y, ".vs.", name_x, ".csv"))
      print(nrow(y[[x]])) }
  }
}
